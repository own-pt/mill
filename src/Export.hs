...
import Data ( WNid(..), WNExtra(..), Relation(..)
            , WordSenseId(..)-- , SynsetId(..)
            , Synset(..), Validated
            , SynsetRelation(..), WSense(..), WordPointer(..)
            , unsafeLookup
            --, idLexFile, lexicographerFileIdToText, singleton, synsetId, synsetType
            --, tshow, extraFrames
            -- , WNPOS(..), WordSenseForm(..)
            )
--import Validate (ValIndex, Index, indexKey, lookupIndex)
---
...
synsetToJSON textToCanonicNames _
  ...
  where
    ...
    -- isHeadRelation (SynsetRelation (Relation "sim" _)) = True
...
--            }
--   where
--     toWord (WSense (WordSenseId WNid{lexForm = WordSenseForm form}) _ _) = (form, lexicalId)
