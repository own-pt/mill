...
(defun mill--flymake-sentinel (source report-fn)
  (lambda (proc _event)
...
(defun mill-flymake (report-fn &rest _args)
  ...
  (unless (executable-find "mill")
    (error "Cannot find a suitable mill executable"))
    ...
    (save-restriction
      ...
      (setq
       ...
       (make-process
        ...
        :command `("mill" "validate"
		   ,@(when mill-validate-one-wordnet
		       (list "--lang" mill-validate-one-wordnet))
		   ,@(when mill-configuration-directory
		       (list "--config-dir" mill-configuration-directory))
		   ,(buffer-file-name))
        :sentinel (mill--flymake-sentinel source report-fn))))))

(defun mill-setup-flymake-backend ()
  (add-hook 'flymake-diagnostic-functions 'mill-flymake nil t))

(provide 'mill-flymake)
;;; mill-flymake.el ends here
